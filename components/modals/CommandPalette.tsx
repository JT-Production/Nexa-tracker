...
export function CommandPalette() {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-xl rounded-2xl border border-slate-200 shadow-2xl overflow-hidden bg-white"
      ...
      >
            ...
            searchResults.map((item, index) => (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-left transition-all cursor-pointer ${
                  index === selectedIndex
                    ? 'bg-indigo-50 text-indigo-950 border border-indigo-200/80 shadow-2xs'
        ...
        <div className="px-4 py-2 border-t border-slate-100 bg-slate-50/70 flex items-center justify-between text-[11px] text-slate-500">
          ...
          <div className="font-mono text-[10px] text-indigo-600 font-medium">Nexa Command Engine</div>
        </div>
      </div>
    </div>
  );
